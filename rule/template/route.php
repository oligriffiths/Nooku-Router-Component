<?php
/**
 * The `RouterRoute` class represents a single URL pattern which is matched against incoming requests, in
 ...
 * Router::match(array("option" => "com_users", "view" => "login")); // returns "/users/login"
 * }}}
 *
 ...
 *
 * {{{$route = RouterRule(array(
 *		'template' => '/users/{:user}',
 *		'pattern' => '@^/u(?:sers)?(?:/(?P<user>[^\/]+))$@',
 *		'params' => array('view' => 'users', 'action' => 'index'),
...
class RuleTemplateRoute
{
	 ...
	 * to match. These must be in the format of KRequest::get parameters. Eg. post.param => value
	 *
	 * @var array
	 */
	...
	public function __construct(Library\ObjectConfig $config = null)
	{
		...
		{
			$key = '_'.$key;
			if(in_array($key, $vars)) $this->$key = $value;
		}
	 ...
	 * @return mixed
	 */
	public function match(Library\HttpUrl $url, $context = null) {

		if (!$query = $this->_matchKeys($url->query)) {
			return false;
		}
		foreach ($this->_subPatterns as $key => $pattern) {
			if (isset($query[$key]) && !preg_match("/^{$pattern}$/", $query[$key])) {
				return false;
			}
		}

		return $this->_write($url, $query, $this->_defaults);
	}
	...
	protected function _matchKeys($options) {
		...
		if (!isset($this->_config['exact']) || !$this->_config['exact']) {
			if (array_intersect_key($this->_keys, $options + $this->_defaults) != $this->_keys) {
				return false;
			}
		} else {
		...
		$base = $this->_keys;
		$match = array_intersect_key($this->_keys, $options + $this->_defaults);
		sort($base);
		sort($match);
	 ...
	 */
	protected function _write(Library\HttpUrl $url, $options, $defaults) {
		...
		foreach (array_reverse($this->_keys, true) as $key) {
			...
			//Remove defaults from the route template if they match the route
			...
			if (isset($this->_config['formatters'][$key])) {
				$value = call_user_func($this->_config['formatters'][$key], $value);
			}
			if ($value === null) {
				$template = str_replace("/{$rpl}", '', $template);
				continue;
		...
		$options = array_diff_key($options, $this->_match);
		$options = array_diff_key($options, $this->_keys);
